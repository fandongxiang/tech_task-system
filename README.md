...
2. 电脑端顶部导航栏设计成模块导入，不采用`iframe`嵌套（*待追加*）
...
1.1 参数按照`片区`、`合同`、`参数类型`、`说明`进行提交，并能够新增合同；
1.2 参数提交页面除提供预览页外，左侧生成上次提交的参数渲染，并将不同的参数背景色单独标出；
1.3 历史提交区，可查看当前片区提交的参数记录，并实现删除（片区选择绑定change事件）；
