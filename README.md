...
1.2 按提交人负责片区，自动选择片区，避免每次手动下拉选择片区（*待优化*）
2. **异常炉台汇总**：
2.1 批量更新
2.2 自动更新运行时间
2.3 自动标绿等径炉台
...
1. **工艺变更提交**
1.1 工艺按`片区`、`变更目的`、`变更内容`、`变更分类`、`影响分类`提交工艺变更项，并实现删除（*待优化更新不删除、复选变更影响*）
1.2 工艺变更项追加效果反馈（*待追加*）
1.3 变更内容和目的文本框，可实现文字加粗等效果（*待追加*）
2. **工艺时间线**
1.1 按`片区`、`变更分类`、`变更影响`对工艺变更进行筛选；
