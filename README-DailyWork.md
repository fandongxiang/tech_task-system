...
3. 请求模块化：将合同请求(`getContractMessage()`)、参数请求(`getSubmitMessage()`)、对比功能(`compareArgumets()`)独立成函数，方便后续结构修改；
4. 请求渲染顺序：合同请求`ajax:success`函数中嵌套参数请求顺序，参数请求`ajax:success`函数中嵌套参数对比函数（因为success后才能获取合同、提交说明）；
5. 页面首次渲染：获取`zoom`默认片区，传入`getContract()`函数渲染；
6. 下拉选择渲染：采用`change`事件，自动渲染，不用点击查询；
7. 默认参数渲染：选择片区无合同或新建合同后，渲染数据库`init`默认参数，方便提交；选择片区后有合同，渲染该合同最近一次提交参数；
8. 历史提交渲染：片区选择后，渲染该片区所有历史提交参数信息；
9. 参数提交功能：为每列参数添加类名，遍历获取值`push`进数组，加入以参数名为键的对象数组中，然后传入后端；
10. 提交说明限制：为提交说明`input`添加`blur`事件，实现失去焦点后发起`ajax`请求获取数据库同片区同合同提交说明，然后通过`some()`与当前值对比，存在则`alert()`和清空输入框；
...
2. 新增提交说明渲染：
  ...
  c. 渲染顺序：必须按照合同请求`ajax:success`中嵌套提交说明请求，提交说明请求`ajax:success`中嵌套参数请求顺序，参数请求`ajax:success`中嵌套参数对比（因为success后才能获取合同、提交说明）；
3. 自动对比当前参数与对比参数：`foreach`嵌套遍历当前和对比参数，且必须放在参数渲染成功后；
4. 生成功率-拉速对比曲线图；
